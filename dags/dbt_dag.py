...
DBT_PROFILES_DIR = "/opt/airflow/dags/dbt_modules"
DBT_PROJECT_DIR = f"{DBT_PROFILES_DIR}/data_warehouse_analytics_engineer"
DBT_TARGET = "dev"
...
with DAG(
    ...
    def make_dbt_task(node, dbt_verb):
        ...
        if dbt_verb == "run":
            ...
            dbt_task = BashOperator(
                task_id=node,
                bash_command=f"python /home/airflow/gcs/dags/dependencies/dbt_runner --no-write-json run --target {DBT_TARGET} --models {model} --profiles-dir {DBT_PROFILES_DIR} --project-dir {DBT_PROJECT_DIR}",
                retries=1,
                retry_delay=timedelta(seconds=10),
            )
        return dbt_task
